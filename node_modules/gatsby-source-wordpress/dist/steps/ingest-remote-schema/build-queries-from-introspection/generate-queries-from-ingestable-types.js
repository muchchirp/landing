...
const generateNodeQueriesFromIngestibleFields = async () => {
  ...
  for (const {
    ...
    const nodesType = typeMap.get((0, _helpers.findNamedTypeName)(nodesField.type));
    if (!nodesType) {
      reporter.panic((0, _formatLogMessage.formatLogMessage)(`Couldn't infer node type in the remote schema from the ${name} root field.`));
    }
    const {
      fields,
      possibleTypes
    ...
    if (process.env.NODE_ENV === `development` && nodesType.name === copyNodeSourcingQueryAndExit) {
      try {
        ...
        await clipboardy.write(_prettier.default.format(nodeListQueries[0], {
          parser: `graphql`
        }));
        process.exit();
      } catch (e) {
